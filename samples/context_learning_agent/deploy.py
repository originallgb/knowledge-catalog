"""Deployment script for Context Learning Agent using environment variables."""
...
def create(service_account: str | None = None) -> None:
    """Creates and deploys the Context Learning agent via AdkApp configuration."""
    agent_dir = os.path.dirname(os.path.abspath(__file__))
    os.chdir(agent_dir)
    logger.info(
        "Deploying Context Learning Agent via AdkApp on Vertex AI Agent Engine..."
    )
    ...
    remote_agent = agent_engines.create(
        app,
        display_name="Context Learning Agent",
        description=(
...
def update(resource_id: str, service_account: str | None = None) -> None:
    ...
    os.chdir(agent_dir)
    logger.info(
        "Updating Context Learning Agent %s via AdkApp on Vertex AI Agent"
        " Engine...",
        resource_id,
    )
